    ...
    <div className="h-28 w-full bg-primary bg-gradient-to-br from-primary to-secondary flex justify-between px-10 border-b-4 border-b-gold text-gold">
      <div className="flex items-center gap-4">
        <Image
          ...
          priority={true}
        />
        <div className="font-mono font-bold text-lg uppercase tracking-wider">
          invite.kz
        </div>
      </div>
      <IoMdMenu
        className="h-8 w-8 self-center hover:cursor-pointer"
            ...
            <div className="flex h-full flex-col justify-between py-2">
              <Sidebar.Items>
                <Sidebar.ItemGroup>
                  ...
                  {categories.map((item) => {
                    const item_text = t(
                      `invitationCategory.name.${item.category}`
                    );
                    return (
                      <Sidebar.Item
                        key={item.id}
                        href={`/${locale}/invitation-categories/${item.id}/edit`}
